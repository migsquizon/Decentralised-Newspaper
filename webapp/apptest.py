...
def test_successful_poll_ending_and_that_publisher_balance_is_credited_because_real_news(eth_tester,w3, foo_contract):
    # send transaction that votes
    tx_hash = foo_contract.functions.submitHeadline("asdasd","asdasd").transact({'from': eth_tester.get_accounts()[2]})
    w3.eth.waitForTransactionReceipt(tx_hash, 180)
    hw = foo_contract.caller.getCurrentHeadline()
    ...
    ## Check if publisher balance is credited so he can claim
    hw = foo_contract.functions.getPublisherBalance().call({'from': eth_tester.get_accounts()[2]})
    assert hw == 2

def test_successful_poll_ending_and_that_publisher_balance_is_NOT_credited_because_fake_news(eth_tester,w3, foo_contract):
    # send transaction that votes
    tx_hash = foo_contract.functions.submitHeadline("asdasd","asdasd").transact({'from': eth_tester.get_accounts()[2]})
    w3.eth.waitForTransactionReceipt(tx_hash, 180)
    hw = foo_contract.caller.getCurrentHeadline()
...
def test_successful_poll_ending_and_that_publisher_balance_credited_and_claim_successful(eth_tester,w3, foo_contract):
    # send transaction that votes
    tx_hash = foo_contract.functions.submitHeadline("asdasd","asdasd").transact({'from': eth_tester.get_accounts()[2]})
    w3.eth.waitForTransactionReceipt(tx_hash, 180)
    hw = foo_contract.caller.getCurrentHeadline()
    ...
    balance = eth_tester.get_balance(eth_tester.get_accounts()[2])
    tx_hash = foo_contract.functions.claimReward(2).transact({'from': eth_tester.get_accounts()[2]})
    receipt = w3.eth.waitForTransactionReceipt(tx_hash, 180)
...
def test_successful_poll_ending_and_that_publisher_balance_is_credited_then_submit_new_headline_again(eth_tester,w3, foo_contract):
    # send transaction that votes
    tx_hash = foo_contract.functions.submitHeadline("asdasd","asdasd").transact({'from': eth_tester.get_accounts()[2]})
    w3.eth.waitForTransactionReceipt(tx_hash, 180)
    hw = foo_contract.caller.getCurrentHeadline()
